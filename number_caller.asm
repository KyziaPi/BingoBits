...
; all functions for calling, displaying, and resetting numbers
...
section .bss
    called_flags resb 76        ; flags for numbers 1–75 (index 1–75)
    called_count resd 1         ; total numbers called

section .text
    global call_number, display_called_numbers, reset_called_numbers
    global called_flags, called_count
    extern rand, printf

; generate and print one random number (1–75)
call_number:
...
display_called_numbers:
    ...
    mov edi, 0          ; counter for newline formatting (10 per line)
...
; clears all called flags and count
reset_called_numbers:
    mov ecx, 76               ; total flags (0–75)
    mov edi, called_flags
.clear_loop:
    mov byte [edi], 0
    inc edi
    loop .clear_loop
